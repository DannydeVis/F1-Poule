...
| `gereden-races.test.mjs` | Wat nog komt boven, gereden races eronder (nieuwste eerst, met wie won); een weekend is pas gereden na de race (of afgelast): na de kwalificatie blijft het bij wat er nog komt, met eronder je punten tot nu toe (met joker) en een knop naar die uitslag, en de joker telt mee in de punten op het racescherm, in de poule en bij een inzending |
| `thema.test.mjs` | Licht of donker: het zonnetje/maantje naast Wissel wisselt meteen, wordt onthouden en al vóór de eerste verf toegepast, gaat terug naar het toestel als je kiest wat het toestel doet, volgt het toestel zolang je niets koos, kleurt de vraag over Google Analytics mee, en duwt de poulenaam niet van het scherm |
| `story.test.mjs` | Het deelplaatje voor een story: kiezen tussen Bericht en Story (onthouden), 1080 bij 1920 met een eigen naam, delen en opslaan, jouw weekend (punten met joker, winst, plek, precies goed, podium) en jouw plek in de stand, een lange racenaam, alleen spelen, halverwege het weekend, en dat alle tekst in de veilige zone staat zonder elkaar te raken |
| `automatisch-invullen.test.mjs` | Automatisch invullen bij vergeten: uit is uit, aanzetten geldt alleen vanaf nu, een willekeurige top 10 (tien verschillende coureurs, eigen lijst per speler en per sessie, vast na herladen en ongeacht de volgorde in de database), telt als een gewone lijst maar wint geen weekend, zegt overal dat hij automatisch is, en races van vóór de overstap houden de WK-stand. Sinds 27 september ook alles wat iemand liet liggen: de losse vragen van race en kwalificatie, de duels en de sprint, met het etiket achter elke kop; een automatisch antwoord krijgt geen contrair-vermenigvuldiger en maakt andermans keuze niet minder zeldzaam; een race van vóór die datum krijgt geen losse vragen erbij |
...
| `puntenkoppen.test.mjs` | De punten per onderdeel op het racescherm: elke top 10, losse vraag en de duels hebben dezelfde kop "behaald van max punten", in je eigen uitslag en bij een ander; de koppen tellen op tot het grote getal; de kop van de top 10 is de som van de regels (bij een sprint de helft, met de reden erbij); een duel toont punten naar rato in plaats van een vinkje; een vraag zonder antwoord is nul van zoveel; met de contrair-vermenigvuldiger staat er alleen wat je kreeg; en een plus voor elk getal dat punten opleverde, ook bij een ander |
| `sprintweekend.test.mjs` | De derde sessie: dat een gewoon weekend er geen tab bij krijgt, dat een sprintweekend er wel een heeft en vóór de kwalificatie, dat hij voor halve punten telt, en dat automatisch invullen hem vóór 27 september oversloeg (op vaste data, zodat de test niet vanzelf verloopt) |
...
| `agenda.test.mjs` | De agenda met alle deadlines: dat hij aan de standaard voldoet, dat een item na een verzetting vervangen wordt in plaats van verdubbeld, dat een race zonder tijd geen item in 1970 oplevert, en dat elk item een melding een uur van tevoren heeft, ook in het `kalender.ics` dat online staat |
| `eerste-indruk.test.mjs` | Wat een vreemde ziet die zonder uitnodiging binnenkomt: uitleg voor wie hem nodig heeft, weg voor wie niet, en het codeveld blijft bovenaan. Plus het PWA-manifest, de pictogrammen, en onder de agendaknop hoe je de melding op een iPhone houdt |
...
| `site.test.mjs` | De landingspagina in zeven talen: wat GitHub Pages publiceert (de site wel, de documenten, tests, scripts en SQL niet, en elk nieuw `.md` ook niet; de testservers passen dezelfde uitsluiting toe), de titel met de zoekterm vooraan en het merk achteraan, de sitenaam met zijn andere namen, de gidsen uit `site/paginas.mjs` met dezelfde keuring als een voorpagina (plus hreflang per cluster, kruimelpad gelijk aan de JSON-LD, zichtbare datum gelijk aan dateModified, geen plekhouder of streepje, ook niet achter een dichte FAQ-vraag, de getallen uit de app per gids; een rekenvoorbeeld is regel voor regel wat `scoreLijst()` uit de app ervan maakt, een spreadsheetformule geeft voor elke plek en een lege cel wat de app geeft, en de tabellen met losse vragen zijn die van de voorpagina), geen weespagina's, elke gidslink onder het blok van de voorpagina waar hij hoort, de generator is gedraaid, per taal de juiste taal, titel- en omschrijvingslengte, één h1, canonical en wederkerige hreflang, geldige JSON-LD waarvan de FAQ woord voor woord op het scherm staat, de punten uit de app, contrast in licht en donker, geen horizontale scroll en elk woord past in zijn kop. Plus: niets van andere hosts, geen 404, de app op noindex, en sitemap, robots.txt, llms.txt en 404.html; de niveaus (Simpel, Klassiek, Gevorderd) met de vragen en punten uit de presets van de app, startlichten 1-3-5, en het echte aantal vragen in de tekst. En de beweging: blokken schuiven pas in beeld als je erbij bent, tellers eindigen op het echte getal, de voortgangsbalk loopt vol, de lichtkrant wordt niet voorgelezen, de rode regel van de kop staat er vóór, tijdens en na de glans helemaal (letters geteld op een schermafdruk); zonder JavaScript, met minder beweging of zonder het script onderaan staat alles er toch |
| `lastmod.test.mjs` | De datum per pagina in de sitemap en in dateModified: gelijk aan `site/lastmod.json`; een andere tekst geeft alleen die pagina een nieuwe datum, een andere vorm niemand, nog eens draaien niets; vergeten te genereren laat `--controle` zakken, en `--controle` schrijft zelf niets |
